...
Externalization (\LaTeX) and stuff:
...
\def\mandatory%
{\framed[background=color,backgroundcolor=yellow,backgroundcorner=round,radius=3pt,location=low,frame=off]}
\def\optional%
{\framed[background=color,backgroundcolor=cyan,backgroundcorner=round,radius=3pt,location=low,frame=off]}
\def\repetition%
{\framed[background=color,backgroundcolor=orange,backgroundcorner=round,radius=3pt,location=low,frame=off]}

\setupinmargin[outer][style=]
...
{\tt\backslash mor%
\optional{[\{morphism\}]}%
\mandatory{\{name\}}\repetition{\textvisiblespace
\{morphism\}%
\textvisiblespace\{name\}}%
;
}
\inouter{Something that might be also quite long too.
A seconda delle necessità si può far di tutto nella vita.}
\blank
\par
...
\par
{\tt
\{morphism\} = \optional{\{labels\}:}\{arrow\}
}

\par
{\tt
\{labels\} = \{label\} \| \repetition{[label]}
}
